...
from web_admin.models import MiseEchelle
from web_admin.forms import MiseEchelleForm
...
def get_metadata(key = None):
    ...
    data = {
        ...
        'table_title': 'Liste techniques de mise à l\'echelle',
        'section_title': 'Parametrage',
        'section_item_title': ' Mise à l\'echelle',
...
def mise_echelle(request):
    init.mise_echelle()
    items = MiseEchelle.objects.all()
...
def mise_echelle_update(request, pk):
    entity = get_object_or_404(MiseEchelle, pk=pk)
    if request.method == 'POST':
        form = MiseEchelleForm(request.POST, instance=entity)
    else:
        form = MiseEchelleForm(instance=entity)
