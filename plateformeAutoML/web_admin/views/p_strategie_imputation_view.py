...
from web_admin.models import StrategieImputation
from web_admin.forms import StrategieImputationForm
...
def get_metadata(key = None):
    ...
    data = {
        ...
        'table_title': 'Liste des strategies d\'imputation',
        'section_title': 'Parametrage',
        'section_item_title': 'Strategie Imputatation',
...
def strategie_imputation(request):
    init.strategie_imputation()
    items = StrategieImputation.objects.all()
...
def strategie_imputation_update(request, pk):
    entity = get_object_or_404(StrategieImputation, pk=pk)
    if request.method == 'POST':
        form = StrategieImputationForm(request.POST, instance=entity)
    else:
        form = StrategieImputationForm(instance=entity)
