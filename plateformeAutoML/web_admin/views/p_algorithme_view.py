...
def get_metadata(key = None):
    ...
    data = {
        ...
        'section_item_title': 'Algorithmes du Machine Learning',
...
def algorithme_create(request):
    if request.method == 'POST':
        form = AlgorithmeForm(request.POST)
    else:
        form = AlgorithmeForm()
    
    return save_entity_form(request, form, get_metadata('path_form_create'))

def algorithme_update(request, pk):
    entity = get_object_or_404(Algorithme, pk=pk)
    if request.method == 'POST':
        form = AlgorithmeForm(request.POST, instance=entity)
    else:
        form = AlgorithmeForm(instance=entity)
    return save_entity_form(request, form, get_metadata('path_form_update'))

def algorithme_delete(request, pk):
    entity = get_object_or_404(Algorithme, pk=pk)
    data = dict()
    context = get_metadata()
