...
from web_admin.models import Encodage
from web_admin.forms import EncodageForm
...
def get_metadata(key = None):
    ...
    data = {
        ...
        'table_title': 'Liste des encodages',
        'section_title': 'Parametrage',
        'section_item_title': ' Encodage',
...
def encodage(request):
    init.encodage()
    items = Encodage.objects.all()
...
def encodage_update(request, pk):
    entity = get_object_or_404(Encodage, pk=pk)
    if request.method == 'POST':
        form = EncodageForm(request.POST, instance=entity)
    else:
        form = EncodageForm(instance=entity)
