...
from web_admin.models import StrategieEncodage
from web_admin.forms import StrategieEncodageForm
...
def get_metadata(key = None):
    ...
    data = {
        ...
        'table_title': 'Liste des strategies d\'encodage',
...
def strategie_encodage(request):
    init.strategie_encodage()
    items = StrategieEncodage.objects.all()
...
def strategie_encodage_update(request, pk):
    entity = get_object_or_404(StrategieEncodage, pk=pk)
    if request.method == 'POST':
        form = StrategieEncodageForm(request.POST, instance=entity)
    else:
        form = StrategieEncodageForm(instance=entity)
