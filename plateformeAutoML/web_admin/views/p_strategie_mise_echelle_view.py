...
from web_admin.models import StrategieMiseEchelle
from web_admin.forms import StrategieMiseEchelleForm
...
def get_metadata(key = None):
    ...
    data = {
        ...
        'table_title': 'Liste des strategies de mise à l\'echelle',
        'section_title': 'Parametrage',
        'section_item_title': 'Strategie Mise à l\'echelle',
...
def strategie_mise_echelle(request):
    init.strategie_mise_echelle()
    items = StrategieMiseEchelle.objects.all()
...
def strategie_mise_echelle_update(request, pk):
    entity = get_object_or_404(StrategieMiseEchelle, pk=pk)
    if request.method == 'POST':
        form = StrategieMiseEchelleForm(request.POST, instance=entity)
    else:
        form = StrategieMiseEchelleForm(instance=entity)
